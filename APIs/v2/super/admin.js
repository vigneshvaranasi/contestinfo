...
const {Users, Students, Performances} = require('../../../db/index.js');
...
router.delete('/deleteBatch', async(req,res)=>{
    try{
        ...
        const allStudentsOfBatch = Students.find({
            year, branch
        });

        const deletedPerformances = Performances.deleteMany({
            rollNo:{
                $in:allStudentsOfBatch.map((student)=>student.rollNo)
            }
        });
        const deletedBatch = Students.deleteMany({year, branch});      
